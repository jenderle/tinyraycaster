...
int main() {
	...
	std::vector<uint32_t> framebuffer(win_w*win_h, 255); // initialize to red
	...
	for(size_t j = 0; j<win_h; j++) {
		for(size_t i = 0; i<win_w; i++) {
			uint8_t r = 255*j/float(win_h); // varies from 0-255 with vertical
			uint8_t g = 255*i/float(win_w); // varies from 0-255 with horizontal
			uint8_t b = 0;
			framebuffer[i+j*win_w] = pack_color(r,g,b);
		}
	}

	const size_t rect_w = win_w/map_w; // width of a cell; relation of map to image size
